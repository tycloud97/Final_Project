resource "proxmox_vm_qemu" "control_plane" {
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      ...
      ansible-playbook playbooks/inventory.yml --extra-vars="host_type=new_control_plane host_ip=$VM_IP host_state=present"
    EOT
  }
...
resource "null_resource" "control_plane-pre-destroy" {
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
        ...
        ansible-playbook playbooks/inventory.yml --extra-vars="host_type=control_plane host_ip=$VM_IP host_state=absent"
    EOT
  }
}
